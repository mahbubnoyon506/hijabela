...
export function Banner({
  ...
  return (
    <div
      className={cn("group/banner relative w-full overflow-hidden", className)}
    >
      <Slider
        ref={sliderRef}
        dots={false}
        ...
        {slides.map((slide, index) => (
          <div key={slide.id}>
            <div className={cn("relative w-full", heightClassName)}>
              ...
              {/* Legibility gradient */}
              {/* {slide.type === "image" ? (
                <div className="absolute left-0 inset-0 bg-linear-to-t from-black/85 via-black/45 to-black/10" />
              ) : null} */}
              ...
              <div className="relative z-10 flex h-full flex-col justify-end gap-4 px-6 pb-14 sm:px-10 sm:pb-16 md:px-8">
                ...
                <h2 className="max-w-2xl leading-tight drop-shadow-sm text-primary text-xl md:text-2xl lg:text-4xl font-light tracking-[2.9px]">
                  {slide.heading}
                </h2>

                {slide.subheading && (
                  <p className="max-w-2xl font-secondary text-base tracking-[2px] text-primary/90">
                    {slide.subheading}
                  </p>
                )}
                      ...
                      return (
                        <Link
                          ...
                          className={cn(
                            ...
                            isSolid
                              ? "bg-linear-to-r from-white to-yellow-500 text-blue-950 shadow-sm hover:from-white-200 hover:to-yellow-400"
                              : "border border-primary/60 text-primary hover:bg-white/10",
                          )}
                        >
                          {cta.label}
                          {showIcon && (
